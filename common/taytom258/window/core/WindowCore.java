...
public class WindowCore extends Window {
	...
	public static class collect extends Window {
			...
			for (String element : str1) {
				if (getTabbedPane().indexOfTab(element) != -1) {
					getTabbedPane().setTitleAt(
							getTabbedPane().indexOfTab(element), "Action");
					getTabbedPane().setEnabledAt(
							getTabbedPane().indexOfTab("Action"), false);
		...
		private static void genCollect() {
			...
			Collection.tsoSubject = textField_TsoSubject.getText()
					.toUpperCase().trim();
			...
			if (getRadioButton_Other().isSelected()) {
				Collection.cmo = textField_CmoName.getText().toUpperCase()
						.trim();
				Collection.cmoDsn = textField_Dsn.getText().toUpperCase()
						.trim();
				Collection.cmoComm = textField_Comm.getText().toUpperCase()
						.trim();
			...
			// Collection.extraComments =
			// getTextArea_Comments().getText().toUpperCase().trim();
		}

		public static void getAction() {

			String[] str1 = textField_TsoSubject.getText().toUpperCase()
					.split("-");
			if (str1.length > 1) {
				String str2 = str1[1];
				...
				if (str2.length() < 3 && str2.equals("01")) {
					getTabbedPane().setTitleAt(
							getTabbedPane().indexOfTab("Action"), "Start");
					getTabbedPane().setEnabledAt(
							getTabbedPane().indexOfTab("Start"), true);
					cardSwap("panel_Start");
					// Collection.start = true;
				} else if (str2.length() < 3 && str2.equals("99")) {
					getTabbedPane().setTitleAt(
							getTabbedPane().indexOfTab("Action"), "Disco");
					getTabbedPane().setEnabledAt(
							getTabbedPane().indexOfTab("Disco"), true);
					cardSwap("panel_Disco");
					// Collection.disco = true;
				} else if (str2.length() < 3 && str2.length() > 1) {
					getTabbedPane().setTitleAt(
							getTabbedPane().indexOfTab("Action"), "Change");
					getTabbedPane().setEnabledAt(
							getTabbedPane().indexOfTab("Change"), true);
					cardSwap("panel_Change");
					// Collection.change = true;
				} else if (str2.length() == 3 && str2.matches(".*[a-yA-Y].*")) {
					getTabbedPane().setTitleAt(
							getTabbedPane().indexOfTab("Action"), "Amend");
					getTabbedPane().setEnabledAt(
							getTabbedPane().indexOfTab("Amend"), true);
					cardSwap("panel_Amend");
					// Collection.amend = true;
				} else if (str2.length() == 3 && str2.matches(".*[zZ].*")) {
					getTabbedPane().setTitleAt(
							getTabbedPane().indexOfTab("Action"), "Cancel");
					getTabbedPane().setEnabledAt(
							getTabbedPane().indexOfTab("Cancel"), true);
