...
public class IERParser {
	...
	public static TreeMap<String, String> parseIER(String t, boolean auto){
		 ...
		 //text sanitization:
		 t = t.replaceAll("\\r\\n|\\n|\\r", LBC);      //remove line breaks and replace with LBC (see //replace strings)
		 ...
		 if(t.indexOf(DTG) == -1){
			 ier.put("Report Date", t.substring(t.indexOf(FM) - ReportLength, t.indexOf(FM)));
		 }else if(t.indexOf(DTG) > -1){
			 ier.put("DTG", t.substring(t.indexOf(DTG) + DTG.length(), t.indexOf(DTG) + DTG.length() + DTGLENGTH));
			 ier.put("Report Date", t.substring(t.indexOf(FM) - ReportLength, t.indexOf(FM)));
		 }
			 ...
			 for (int i=0; i<items.length; i++) {
			  if (items[i] != skippy) {
			   ...
			   if (st.indexOf(bs) - (st.indexOf(as) + as.length()) > 0) {
				   ier.put(items[i], st.substring(st.indexOf(as) + as.length(), st.indexOf(bs)));
				   st = st.substring(st.indexOf(ier.get(items[i])) + ier.get(items[i]).length(), st.length());
			   }else{
				   ier.put(items[i], "N/A");
